...
class ResourceTagLib {
    ...
    static final String REQ_ATTR_PREFIX_PAGE_FRAGMENTS = 'resources.plugin.page.fragments'
    static final String REQ_ATTR_PREFIX_AUTO_DISPOSITION = 'resources.plugin.auto.disposition'
    private static final List fragmentTypes = ['script', 'style']

    static stashWriters = [
        script: { out, stash ->
            writeStash '<script type="text/javascript">', '</script>', out, stash
        },

        style: { out, stash ->
            writeStash '<style type="text/css">', '</style>', out, stash
        }
    ]

    protected static void writeStash(String start, String end, out, stash) {
        out << start
        for (s in stash) {
            out << s
        }
    ...
    private stashPageFragment(String type, String disposition, fragment) {
        if (log.debugEnabled) {
    ...
    private consumePageFragments(String type, String disposition) {
        request[makePageFragmentKey(type, disposition)] ?: Collections.EMPTY_LIST
    }
    ...
    private isAutoLayoutDone(disposition) {
        request[makeAutoDispositionKey(disposition)]
    }

    private autoLayoutDone(disposition) {
        request[makeAutoDispositionKey(disposition)] = true
    }
