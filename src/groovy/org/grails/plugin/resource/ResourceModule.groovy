...
class ResourceModule {
    ...
    ResourceModule(String name, ResourceProcessor svc) {
        this.name = name
        pluginManager = svc.pluginManager
        defaultBundle = false
    }

    ResourceModule(String name, Map resourceInfo, defBundle, ResourceProcessor svc) {
        this(name, svc)
        ...
        resources << newResourceFromArgs(args, svc, true)
        lockDown()
    }
    ...
    ResourceModule(name, List resourceInfoList, defBundle, svc) {
        ...
        resourceInfoList.each { i ->
            if (i instanceof Map) {
                ...
                resources << newResourceFromArgs(url:i, svc, resourceInfoList.size()==1)
            } else {
    ...
    void lockDown() {
        resources = resources.asImmutable()
    }

    Set<String> getRequiredDispositions() {
        if (!dispositions) {
            dispositions = (resources.findAll { r -> r.disposition }).disposition as Set
        }
        dispositions
    }
}
